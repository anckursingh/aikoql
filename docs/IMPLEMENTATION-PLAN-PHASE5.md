...
Current state: parser+AST+semantic+planner SHIPPED (P3-M4: TRAVERSE DEPTH, INGEST, ident `:` fix; grammar_coverage 42/42, golden 13/13, fuzz 4/4); MATCH/WHERE/RETURN, SIMILAR TO, AS OF, evidence predicates, projection all parse and execute. Missing: ORDER BY / LIMIT / OFFSET; GROUP BY + aggregates (COUNT/SUM/AVG/MIN/MAX) — syntax lands here, execution in P5-M5; JOIN … ON — parse-only, executes in P5-M6; stable-AST contract; the explain contract P5-M3 fills. **Known defect (found by the P5-M0 oracle 2026-09-13):** numeric literals lower to `Value::Float` (parser/mod.rs:381), so `WHERE temp == 35` compares Float(35.0) against Int-seeded properties — the runtime deems the types incomparable and the result is silently empty. The M0 corpus works around it with Text predicates; the fix is M2's kq010.
