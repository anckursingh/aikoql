...
| Agent efficacy benchmark (A–D treatments, G10) | ✅ | `agent_efficacy_bench.rs` — 20 engineering tasks (AGENT-001/002/004 shapes) over the real docs corpus (20 `docs/*.md` → 1546 chunks → merged graph 7302 entities/2778 facts/5340 relations), 4 treatments, mechanical judge, **measured 2026-08-23 with llama3.1 (GPU-offloaded Ollama): success A 0.200\*/B 0.750/C 0.500/D 0.150** (\*A's 4 passes are all token-overlap guess passes — strict judge 0), input tokens/query 28/1237/1273/320, LLM calls 20/20/20/0 (SEM-003), tool calls 0/0/0/20, latency 23.5/27.7/13.0/8.5 s/query, cost $0.0013/$0.0049/$0.0050/$0.0010. Findings: the lexical RAG pack beats the code-graph expansion on this corpus (dilution; G11's Track-B favored C — corpus-dependent); AIKOQL wins cost/determinism but is table-blind — `markdown.rs` has no table extraction, so answers living in doc tables (17/20 goldens) never enter the IR (3/3 D hits are prose phrases) — flags P1 extraction; the repo-only agent hallucinates confidently (BNC). Judge caveat: ≤1-missing-token is lenient on 2-token goldens (hardening = follow-up) |
...
| # | Gap | Suite IDs | Effort | Notes |
| --- | --- | --- | --- | --- |
| G10 | **Agent efficacy benchmark** — 50–100 engineering tasks, treatments A (repo-only) / B (RAG memory) / C (code graph) / D (AIKOQL), measured success/tokens/tool calls | §31, AGENT-* | ✅ done (2026-08-23) | `agent_efficacy_bench.rs`: 20 tasks v1 over the real docs corpus, A/B/C/D treatments, mechanical judge, live local model (answer_gen seam). **Measured with llama3.1: success A 0.200\*/B 0.750/C 0.500/D 0.150** (\*all 4 A passes are token-overlap guesses — strict judge 0), input tokens 28/1237/1273/320, LLM calls 20/20/20/0, tool calls 0/0/0/20, latency 23.5/27.7/13.0/8.5 s/query, cost $0.0013/$0.0049/$0.0050/$0.0010. **Findings:** RAG memory wins accuracy here (graph expansion dilutes — G11's Track-B favored C, corpus-dependent); AIKOQL is cheapest + 0 LLM but table-blind — `markdown.rs` has no table extraction, so table-bound answers (17/20 goldens) never enter the IR (3/3 D hits are prose phrases) — the flagship P1-extraction signal; A hallucinates confidently. Scope: AGENT-003/005 (implement/safe-execution) need agent loops — deferred; 50–100 task scale = corpus extension. Gates structural only (model answers are not CI-pinnable); verdict printed |
