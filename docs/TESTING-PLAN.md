...
| Agent efficacy benchmark (A–D treatments, G10) | ✅ | `agent_efficacy_bench.rs` — 20 engineering tasks (AGENT-001/002/004 shapes) over the real docs corpus (20 `docs/*.md` → 1550 chunks → merged graph 7505 entities/4922 facts/3719 relations), 4 treatments, mechanical judge, **measured 2026-08-23 with llama3.1 (GPU-offloaded Ollama): success A 0.200\*/B 0.750/C 0.650/D 0.350** (\*A's 4 passes are all token-overlap guess passes — strict judge 0), input tokens/query 28/1338/1339/354, LLM calls 20/20/20/0 (SEM-003), tool calls 0/0/0/20, latency 23.0/26.7/16.8/8.1 s/query, cost $0.0013/$0.0052/$0.0052/$0.0011. Findings: the lexical RAG pack still wins accuracy here (graph expansion dilutes vs G11's Track-B — corpus-dependent); **P1 markdown table extraction landed (2026-08-23): GFM pipe tables → Table AST nodes → the fragment leg's cell-cited facts (row-phrase anchors, `TableCell` evidence) — D 0.150 → 0.350 (+4 table-bound goldens: knowledge_bench.rs, e2e_answer_quality.rs, measured-results row, Track-B corpus), C 0.500 → 0.650 (table facts enrich its packs); remaining D misses are entity-gate/ranking (row-phrase anchors don't share question vocabulary), not extraction**; the repo-only agent hallucinates confidently (BNC). Judge caveat: ≤1-missing-token is lenient on 2-token goldens (hardening = follow-up) |
...
| # | Gap | Suite IDs | Effort | Notes |
| --- | --- | --- | --- | --- |
| G10 | **Agent efficacy benchmark** — 50–100 engineering tasks, treatments A (repo-only) / B (RAG memory) / C (code graph) / D (AIKOQL), measured success/tokens/tool calls | §31, AGENT-* | ✅ done (2026-08-23) | `agent_efficacy_bench.rs`: 20 tasks v1 over the real docs corpus, A/B/C/D treatments, mechanical judge, live local model (answer_gen seam). **Measured with llama3.1: success A 0.200\*/B 0.750/C 0.650/D 0.350** (\*all 4 A passes are token-overlap guesses — strict judge 0), input tokens 28/1338/1339/354, LLM calls 20/20/20/0, tool calls 0/0/0/20, latency 23.0/26.7/16.8/8.1 s/query, cost $0.0013/$0.0052/$0.0052/$0.0011. **Findings:** RAG memory wins accuracy here (graph expansion dilutes — G11's Track-B favored C, corpus-dependent); **the P1 table-blindness is fixed — GFM pipe tables now enter the IR as cell-cited facts (row-phrase anchors, `TableCell` evidence), D 0.150 → 0.350 with +4 table-bound goldens, C 0.500 → 0.650; the remaining 13 D misses are entity-gate/ranking (anchors don't share question vocabulary), not extraction**; A hallucinates confidently. Scope: AGENT-003/005 (implement/safe-execution) need agent loops — deferred; 50–100 task scale = corpus extension. Gates structural only (model answers are not CI-pinnable); verdict printed |
