...
- **Unit**: 364 lib tests (+11): 4 PDF (`flate→PGM wrap`, `jpx+ccitt raw`, `vector-only positive`, `text-mixed negative`) + 2 docx (`structure walk` incl. gridSpan/hyperlink/caption/page-2 image, `tag-strip fallback`) + 1 `ast::tests` (`ATX headings classify with levels`) + 1 html (`structure walk` incl. loose-td tables, both list kinds, link, img+alt, script/head dropped) + 3 `visual::tests` (`chart_specialist_fills_axes_series_from_adjacent_table` incl. unit-split y_axis, `chart_specialist_leaves_chart_without_adjacent_table_unfilled`, `split_unit_parses_label_and_unit`). Existing docx/html text tests re-verified against the structured parsers.
- **Acceptance**: 10/10.
- **Gates**: `cargo fmt --all` clean, `cargo clippy --workspace --all-targets` clean (0 warnings), `cargo test --workspace` green (all crates, exit 0).

### Next implementation — PR-F completion continues, then PR-C

Remaining "do not skip anything" items, in order: OCR fill for Screenshot/ScannedText (tesseract → `ImagePayload.ocr_text`), inline markdown images → Image nodes, and the VLM analyzer set behind the traits (§13/§32, env-configured, not default — DoD row 14). Then PR-C: document-hash fragment-id prefix (consumes `DocumentAst.document_id`, landed in PR-B). PR-G stays gated on the rule-baseline benchmarks (§60).
