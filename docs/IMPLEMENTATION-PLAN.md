...
Q1: fail-closed middle corruption is correct HERE because the WAL has no redundant framing — any alternative (skip, truncate) silently discards acknowledged commits; the tail case is preserved by the resync classifier. Q2: single-writer is not an MVP bottleneck — AIKOQL's mutation pattern is one kernel committing batches through one pipeline; interactive/ingestion rates are fsync-bound, and KSE-13 measured writer latency ≈ 4.5-6 ms dominated by durability, not contention; 120C will put a number on the ceiling. Q3: full replay is acceptable for MVP at the 100K-KO boundary (92.4 ms at 2,200 records scales linearly; the 142 matrix measures the real curve) — unacceptable for unbounded history, which §13 already excludes from certification. Q4: compaction must NOT block MVP release — no phase mandates it, WAL growth is the documented limitation, and the adoption verdict (2.90× ingest) already accounts for replay-at-open; it becomes P0 when a measured MVP workload's replay time or disk exceeds the published boundary.
