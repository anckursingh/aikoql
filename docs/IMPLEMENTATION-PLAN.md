...
**Next session:** v0.3 release decision (the user's call — dogfood gate PASSED 2026-08-19, all 10 continuity questions answered with lineage over the ingested repo; PR #1 carries K1–K5). EVO-003 closed 2026-08-25 — `apply_schema_migration` op (rename/set-default transforms, version gate, fail-closed authz, atomic via transact with schema-registry rollback, idempotent re-apply) + codec wire v1 envelope (`encode_ko_wire`/`decode_ko_wire`, legacy bytes forever readable, canonical `encode_ko` frozen for the audit chain); 13 tests (t06zx–t06zzf + `wire_codec_*`), certification.rs EVO-003 row now covered. Open items after certification GO (2026-08-25): RET-CHAT-001 retention/expiry (G13), §38–39 conversation summarization, §40 memory compression, MEM-001 working memory, PROG-CHAT discovery/approval, AGENT-003/005 agent loops, MRFC-0070 A2 non-Rust parsers, MRFC-0060 ~5% polish (index-backed uniqueness, conditional uniqueness, pushdown verification), KVM bytecode, temporal-policy retrieval item (G11 measured 0.0 suppression)
