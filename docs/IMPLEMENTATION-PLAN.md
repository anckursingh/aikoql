...
**Status (2026-08-31):** M1–M3 complete — contract conformance (KSE-001..006, 19/19 across three backends), key semantics (KSE-010..017 over the real kernel, 8/8), record envelope (KSE-020..023 incl. file-level corruption/torn-tail reopen, 10/10), block abstraction (KSE-030..033, 4/4). M4 in progress: KSE-5 done (KSE-040 locality measured across redb/RocksDB/Aikoql — report `artifacts/storage-engine/kse5-locality.md`; Aikoql fastest reads, 0-disk retrieval, 1.55× WAL amplification, replay-at-open is the known ponytail). KSE-6 done (KSE-050..052 adjacency traversal — report `artifacts/storage-engine/kse6-relationship-locality.md`: 1 scan per lookup at every fan-out on every backend, all 11,111 edges verified bidirectional, Aikoql fastest at every size; verdict: the relo/reli prefix layout already IS the knowledge-aware adjacency — no packed adjacency prototype built). KSE-7 (temporal locality) remains.
