...
| 17 | Existing ingestion tests green | ✅ 417 lib (+418 with `--features transform`) + 11 acceptance + 10-fixture golden suite + retrieval-quality gate (15 queries × 4×3 §60 matrix + 3 visual queries × 4 corpora; visual R@1/3/5 = 1.000, hybrid recall@5 = 0.933) + e2e + doc-tests |
...
**Ponytail boundaries (documented ceilings):** classification is keyword-heuristic — the `VisualClassifier`/`*Analyzer` traits are the VLM seams (HLD §32); no model is called today. Formula payloads keep the LaTeX verbatim (no TeX AST). Screenshot/ScannedText classify but degrade to Image facts (no OCR-of-screenshot). Inline images mid-paragraph stay raw text (PR-B deferral). HTML single-item ordered lists classify as headings downstream (numeric-prefix heuristic) — multi-item lists group correctly.
...
### Next implementation — OCR/VLM wiring behind the PR-F seams

The extraction surface is now complete for every modality the fixture corpus exercises (raster images, drawn charts, tables, formulas, diagrams, OCR screenshots). What remains is depth, not breadth: wire real providers behind the PR-F seams (`vlm` feature module exists — OCR model/VLM endpoints), populate `DiagramPayload.asset` for visual diagram ranking (reuses PR-N's extraction machinery unchanged), then make the §60 real-model decision — the PR-M `MultimodalEmbeddingProvider` seam is the foundation for it.
