...
| — (plan) | Diagnostics surface | Kernel violation-event ring (VecDeque cap 256, oldest evicted) + MCP `constraint_diagnostics` tool returning events[] + stats{} | cst002/003 via `k.violation_events()`; MCP 130/0; dogfood smoke through the repo-built plugin binary (tool listed, shape pinned, remember alongside) |
