...
- **v1.1 ranking fixes measured (2026-08-23)**: section-heading anchors for cell facts measured negative (D 0.350→0.300 — heading-derived facts evicted the exact cell facts in the budget) and were reverted. Exact-token gate escape shipped instead: facts whose statement shares ≥2 content tokens with the task bypass the entity gate (one shared token stays gated — G12 hoover guard); measured D 0.350→**0.400** (T0 `comparative_cost_bench.rs`), all other gates green. The remaining 12 D misses: 7 paraphrase-gap goldens inside "Location / note" appendix facts (no lexical bridge — statement/snippet enrichment or semantic matching candidates), 5 budget-cut (the entity-mention section starves the facts fold — pack budget rebalancing candidate).
