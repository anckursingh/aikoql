...
| P3-M4 | Compiler completion | §61–63 | ✅ | cpl001–006 green (cpl_compile 4/4, cpl_execution 2/2 — diamond closure {b,c,d,e}@DEPTH 3, {b,c}@default); cpl007 = golden 13/13 + grammar_coverage 41/41 + fuzz 4/4 rows; parser lib 76/76; runtime 23/0; kernel + mcp suites untouched green; fmt + clippy `-D warnings` green; dogfood INGEST round-trip through repo-built plugin (aikoql tool → aikoql:document → document_list) |
