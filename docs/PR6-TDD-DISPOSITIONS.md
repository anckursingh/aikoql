...
| P1-4 seed-determinism gate | DONE (this commit) — `scripts/check-test-env-hygiene.sh`: (file\|token) allowlist pinning the 10 existing set_var sites (each with its honest reason) + zero-allowlist unseeded-RNG leg; CI-wired after check-skip-drift; RED archived `env-hygiene-vs-fake-tree` (fake tree via TESTS_ROOT, exit 1) |
| P1-5 shuffle runs | next |
