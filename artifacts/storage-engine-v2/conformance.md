...
Date: 2026-09-01 · the six KSE-1 asserts from one shared definition (`tests/common` `kse` module, copied verbatim from v1's harness), run per backend as granular tests (`tests/engine.rs`, V2-Adopt) and as this matrix (`kse20_backend_conformance.rs`, KSE-20). All through `&dyn StorageEngine` — no backend-specific type above the boundary (§32).
