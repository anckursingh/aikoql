...
# workflow (CI-02: the shuffle job lives in benchmark.yml, the one
...
grep -q 'tool: nextest' "$wf" || { echo "SHUFFLE: benchmark.yml lacks the nextest install step" >&2; fail=1; }
