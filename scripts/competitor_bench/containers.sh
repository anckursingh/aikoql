...
# method pins. PG is the pgvector image (postgres 16 + the extension — the
# knowledge_query cell's vector leg). CI-08 (§18): explicit version tags
# only — bench.py records the pulled digest into result.json.
#
...
case "${1:-}" in
  start)
    ...
    docker run -d --name bench-qdrant -p 6333:6333 qdrant/qdrant:v1.19.1
