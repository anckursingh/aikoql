...
Engines:
  ...
  postgresql — postgres:16-alpine container, SQL
  neo4j     — neo4j:5-community container, Cypher
...
def bench_pg(ds):
    ...
    conn.execute("CREATE TABLE notes (koid text PRIMARY KEY, topic text, body text)")
...
def bench_neo4j(ds):
    ...
    with driver.session() as s:
        ...
        s.run("UNWIND $rows AS r MERGE (n:Note {koid: r.koid}) "
              "SET n.topic = r.topic, n.body = r.body",
    ...
    workloads = run_engine(
        ...
        [None, None, None, None, build_graph, None])
...
def bench_qdrant(ds):
    ...
    workloads = run_engine(
        connect, lambda c: c.close(),
        [None, None, None, None, None, build_vector])
...
def engine_meta():
    ...
    for name in ["bench-pg", "bench-neo4j", "bench-qdrant"]:
        try:
            img = subprocess.run(
...
def main():
    ...
    kb = Path(tempfile.mkdtemp(prefix="aikoql-bench-"))
