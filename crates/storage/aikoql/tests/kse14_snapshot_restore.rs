...
use aikoql_kernel::{
    ...
    KnowledgeContext, LifecycleState, Metadata, PropertyMap, RedbEngine, RelationshipRef,
    RememberRequest, Schema, Subject, SupersedeRequest, Value, KOID,
};
...
// ---------------------------------------------------------------------------
// Model-free structural sweep: every invariant that must hold at ANY batch
// boundary, computed from the store's own rows (no reference model — needed
// where the capture point is unknown, KSE-132, and re-verifies the byte-exact
// restore, KSE-130/131).
// ---------------------------------------------------------------------------
...
fn structural_sweep(k: &Kernel, engine: &AikoqlStorageEngine, label: &str) {
...
fn verify_byte_exact_and_sweep(
    ...
    structural_sweep(&reopened, dst, label);
    reopened
}
...
fn kse132_snapshot_with_active_writers() {
    ...
    structural_sweep(&reopened, &dst, "kse132");
