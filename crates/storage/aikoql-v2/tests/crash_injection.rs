//! SE2-M36 — crash injection REDs (spec §38 milestone 8): seven
//! FAIL_AFTER windows on the compaction relocation path, each pinned with
//! the child-kill harness and the §38 verification chain — reopen →
//! validate consistency → verify all objects → verify no invalid
//! locations. The windows are the §23 publication order's boundaries:
//!
//! FAIL_AFTER_SEGMENT_WRITE / _FSYNC — merge output written to the temp,
//! never renamed: the pre-compact state governs, no merged segment visible
//! (P5-M39: these park inside the merge staging dir — the marker rides the
//! published file's parent — and the reopen sweeps the residue).
//! FAIL_AFTER_LOCATION_WRITE / _FSYNC — segments renamed (orphans), the
//! relocation log never renamed: old placements stay authoritative.
//! FAIL_AFTER_MANIFEST_WRITE / _FSYNC — the relocation log IS visible past
//! CURRENT (an orphan): the §24 state-C window — old placements govern.
//! FAIL_AFTER_PUBLISH — CURRENT names the merged generation: the §24
//! state-D window — the relocated placements govern.

mod common;

use aikoql_storage_v2::db::{Config, Db};
use aikoql_storage_v2::format::Current;
use aikoql_storage_v2::identity::directory::{IdentityResolver, LocalIdentityDirectory};
use aikoql_storage_v2::identity::topology::{LocalReplicaDirectory, ReplicaDirectory};
use aikoql_storage_v2::identity::{ObjectId, ReplicaId};
use aikoql_storage_v2::placement::directory::{
    placement_log_path, LocalPlacementResolver, Placement, PlacementResolver,
};
use aikoql_storage_v2::segment::{segment_path, SegmentReader};
use common::dir;
use std::path::{Path, PathBuf};
use std::process::{Child, Command, Stdio};
use std::time::{Duration, Instant};

fn oid(byte: u8) -> ObjectId {
    ObjectId([byte; 16])
}

fn rid_of(db: &Db, a: ObjectId) -> ReplicaId {
    let lid = LocalIdentityDirectory::new(db).resolve(a).unwrap().unwrap();
    LocalReplicaDirectory::new(db)
        .resolve_local(lid)
        .unwrap()
        .unwrap()
}

fn placement_of(db: &Db, rid: ReplicaId) -> Option<Placement> {
    LocalPlacementResolver::new(db).resolve(rid).unwrap()
}

/// The deterministic two-flush workload (the cp shape): fixed creation
/// order, so rids/segment ids/anchors/generations reproduce exactly.
fn workload(db: &Db) {
    let a = oid(0xA1);
    let b = oid(0xB2);
    let c = oid(0xC3);
    db.put_object(a, b"a1", b"va1").unwrap();
    db.put_object(a, b"a2", b"va2").unwrap();
    db.put_object(b, b"b1", b"vb1").unwrap();
    db.flush().unwrap();
    db.put_object(a, b"a3", b"va3").unwrap(); // a's newest key
    db.put_object(c, b"c1", b"vc1").unwrap();
    db.flush().unwrap();
}

/// §38 — verify all objects: every key the workload wrote still answers
/// the right value, whatever window the crash left behind.
fn verify_all_objects(db: &Db) {
    let a = oid(0xA1);
    assert_eq!(db.get_object(a, b"a1").unwrap(), Some(b"va1".to_vec()));
    assert_eq!(db.get_object(a, b"a2").unwrap(), Some(b"va2".to_vec()));
    assert_eq!(db.get_object(a, b"a3").unwrap(), Some(b"va3".to_vec()));
    assert_eq!(
        db.get_object(oid(0xB2), b"b1").unwrap(),
        Some(b"vb1".to_vec())
    );
    assert_eq!(
        db.get_object(oid(0xC3), b"c1").unwrap(),
        Some(b"vc1".to_vec())
    );
}

/// §38 — verify no invalid locations: every placement resolves, and every
/// Segment placement names a real entry in a manifest segment (the reopen
/// validator already fails closed on anything else — this walks the whole
/// map explicitly).
fn verify_no_invalid_locations(db: &Db, dir: &Path) {
    let rids = vec![
        rid_of(db, oid(0xA1)),
        rid_of(db, oid(0xB2)),
        rid_of(db, oid(0xC3)),
    ];
    for rid in rids {
        let Some(placement) = placement_of(db, rid) else {
            panic!("replica {rid:?} has no placement after the crash");
        };
        if let Placement::Segment(loc) = placement {
            let entry = SegmentReader::open(&segment_path(dir, loc.segment_id.0))
                .unwrap()
                .entry_at(loc.block_id, loc.entry_offset)
                .unwrap()
                .expect("the placement names an existing entry");
            assert_eq!(entry.replica_id, rid, "the entry carries the replica's id");
        }
    }
}

// ---------------------------------------------------------------------------
// Child-kill harness (KSE-15 pattern, the cp009/010 shape)
// ---------------------------------------------------------------------------

const CHILD_ENV: &str = "AIKOQL_V2_KILL_CHILD";
const DIR_ENV: &str = "AIKOQL_V2_KILL_DIR";
const PLACE_ENV: &str = "AIKOQL_V2_PLACE_PARK";

fn child_dir() -> PathBuf {
    PathBuf::from(std::env::var(DIR_ENV).expect("child dir env"))
}

fn spawn_child(test_name: &str, dir: &Path, stage: &str) -> Child {
    Command::new(std::env::current_exe().expect("current exe"))
        .arg("--exact")
        .arg(test_name)
        .env(CHILD_ENV, "1")
        .env(DIR_ENV, dir)
        .env(PLACE_ENV, stage)
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn child")
}

fn wait_for(path: &Path, timeout: Duration) {
    let start = Instant::now();
    while !path.exists() {
        assert!(
            start.elapsed() < timeout,
            "marker {} never appeared",
            path.display()
        );
        std::thread::sleep(Duration::from_millis(20));
    }
}

/// P5-M39 — the merge's SEGMENT windows park inside its staging
/// directory (the park marker rides the published file's parent, and the
/// merge publishes into `.compact-staging-*`); the LOCATION/MANIFEST/
/// PUBLISH windows still park in the data dir itself.
fn wait_for_staging_park(d: &Path, stage: &str, timeout: Duration) {
    let start = Instant::now();
    loop {
        let found = std::fs::read_dir(d).unwrap().flatten().any(|e| {
            e.file_name()
                .to_string_lossy()
                .starts_with(".compact-staging-")
                && e.path().join(stage).exists()
        });
        if found {
            return;
        }
        assert!(
            start.elapsed() < timeout,
            "marker {stage} never appeared in a staging dir under {}",
            d.display()
        );
        std::thread::sleep(Duration::from_millis(20));
    }
}

fn child_cfg() -> Config {
    let mut cfg = Config::new(child_dir());
    cfg.l0_compact_trigger = 0; // SE2-M10 — the explicit compact's windows only
    cfg
}

/// The scratch replay: reproduces the child's exact workload to learn the
/// placements each window must hold (deterministic — sorted generation
/// assignment, SE2-M35). `compact` = true replays the post-compact state.
fn scratch_placements(name: &str, compact: bool) -> Vec<(ReplicaId, Placement)> {
    // One scratch dir PER test — cargo runs tests in parallel and two
    // tests sweeping the same name would race each other's replay.
    let d = dir(name);
    let ps = {
        let mut cfg = Config::new(d.clone());
        cfg.l0_compact_trigger = 0;
        let db = Db::open(cfg).unwrap();
        workload(&db);
        let rids = vec![
            rid_of(&db, oid(0xA1)),
            rid_of(&db, oid(0xB2)),
            rid_of(&db, oid(0xC3)),
        ];
        if compact {
            db.compact().unwrap();
        }
        rids.into_iter()
            .map(|r| (r, placement_of(&db, r).unwrap()))
            .collect()
    }; // the Db drops here — no handle held when the dir is swept
    std::fs::remove_dir_all(&d).ok();
    ps
}

/// Assert the reopened placements match the expected list exactly.
fn assert_placements(db: &Db, want: &[(ReplicaId, Placement)], why: &str) {
    for (rid, want) in want {
        assert_eq!(placement_of(db, *rid), Some(*want), "{why}");
    }
}

// ---------------------------------------------------------------------------
// The seven windows
// ---------------------------------------------------------------------------

#[test]
fn ci001_fail_after_segment_write() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks inside the first chunk's publish
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci001");
    let pre = scratch_placements("ci001-scratch", false);
    let mut child = spawn_child(
        "ci001_fail_after_segment_write",
        &d,
        "FAIL_AFTER_SEGMENT_WRITE",
    );
    wait_for_staging_park(&d, "FAIL_AFTER_SEGMENT_WRITE", Duration::from_secs(60));
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    // The merged output never renamed out of staging: nothing new
    // visible, the pre-compact state governs, and the reopen sweeps the
    // staged residue.
    let current = Current::read(&d.join("CURRENT")).unwrap();
    let merged = segment_path(&d, current.manifest_generation + 1);
    assert!(!merged.exists(), "the merged segment was never published");
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci002_fail_after_segment_fsync() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks after the chunk temp fsyncs
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci002");
    let pre = scratch_placements("ci002-scratch", false);
    let mut child = spawn_child(
        "ci002_fail_after_segment_fsync",
        &d,
        "FAIL_AFTER_SEGMENT_FSYNC",
    );
    wait_for_staging_park(&d, "FAIL_AFTER_SEGMENT_FSYNC", Duration::from_secs(60));
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    let current = Current::read(&d.join("CURRENT")).unwrap();
    let merged = segment_path(&d, current.manifest_generation + 1);
    assert!(!merged.exists(), "the merged segment was never renamed");
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci003_fail_after_location_write() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks inside the relocation log's publish
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci003");
    let pre = scratch_placements("ci003-scratch", false);
    let mut child = spawn_child(
        "ci003_fail_after_location_write",
        &d,
        "FAIL_AFTER_LOCATION_WRITE",
    );
    wait_for(
        &d.join("FAIL_AFTER_LOCATION_WRITE"),
        Duration::from_secs(60),
    );
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    // Segments renamed (orphans), the relocation log never did: old
    // placements stay authoritative.
    let current = Current::read(&d.join("CURRENT")).unwrap();
    assert!(
        !placement_log_path(&d, current.manifest_generation + 1).exists(),
        "the relocation log was never renamed"
    );
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci004_fail_after_location_fsync() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks after the relocation log temp fsyncs
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci004");
    let pre = scratch_placements("ci004-scratch", false);
    let mut child = spawn_child(
        "ci004_fail_after_location_fsync",
        &d,
        "FAIL_AFTER_LOCATION_FSYNC",
    );
    wait_for(
        &d.join("FAIL_AFTER_LOCATION_FSYNC"),
        Duration::from_secs(60),
    );
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    let current = Current::read(&d.join("CURRENT")).unwrap();
    assert!(
        !placement_log_path(&d, current.manifest_generation + 1).exists(),
        "the relocation log was never renamed"
    );
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci005_fail_after_manifest_write() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks inside the manifest's publish
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci005");
    let pre = scratch_placements("ci005-scratch", false);
    let mut child = spawn_child(
        "ci005_fail_after_manifest_write",
        &d,
        "FAIL_AFTER_MANIFEST_WRITE",
    );
    wait_for(
        &d.join("FAIL_AFTER_MANIFEST_WRITE"),
        Duration::from_secs(60),
    );
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    // The relocation log is visible past CURRENT (the §24 state-C
    // window): reported-and-ignored, the old placements govern.
    let current = Current::read(&d.join("CURRENT")).unwrap();
    assert!(
        placement_log_path(&d, current.manifest_generation + 1).exists(),
        "the relocation log was renamed before the manifest window"
    );
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci006_fail_after_manifest_fsync() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks after the manifest temp fsyncs
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci006");
    let pre = scratch_placements("ci006-scratch", false);
    let mut child = spawn_child(
        "ci006_fail_after_manifest_fsync",
        &d,
        "FAIL_AFTER_MANIFEST_FSYNC",
    );
    wait_for(
        &d.join("FAIL_AFTER_MANIFEST_FSYNC"),
        Duration::from_secs(60),
    );
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    let current = Current::read(&d.join("CURRENT")).unwrap();
    assert!(
        placement_log_path(&d, current.manifest_generation + 1).exists(),
        "the relocation log was renamed before the manifest window"
    );
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &pre, "the pre-compact placements govern");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}

#[test]
fn ci007_fail_after_publish() {
    if std::env::var_os(CHILD_ENV).is_some() {
        let db = Db::open(child_cfg()).unwrap();
        workload(&db);
        db.compact().unwrap(); // parks after CURRENT names the merge
        unreachable!("the parent kills the parked child");
    }
    let d = dir("ci007");
    let post = scratch_placements("ci007-scratch", true);
    let mut child = spawn_child("ci007_fail_after_publish", &d, "FAIL_AFTER_PUBLISH");
    wait_for(&d.join("FAIL_AFTER_PUBLISH"), Duration::from_secs(60));
    child.kill().expect("kill child");
    child.wait().expect("wait child");

    // The §24 state-D window: CURRENT names the merged generation — the
    // relocated placements govern on reopen (the old segment files are
    // still on disk, reported-and-ignored).
    let db = Db::open(Config::new(d.clone())).unwrap();
    assert_placements(&db, &post, "the relocated placements govern after CURRENT");
    verify_all_objects(&db);
    verify_no_invalid_locations(&db, &d);
}
