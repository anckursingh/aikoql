...
impl OntologyDef {
    ...
    pub fn to_property_map(&self) -> PropertyMap {
        ...
        for me in &self.mappings {
            ...
            mm.insert("physical_type".into(), Value::Text(me.physical_type.clone()));
    ...
    pub fn from_property_map(props: &PropertyMap) -> KResult<OntologyDef> {
        let namespace = map_get_str(props, "namespace")
            .unwrap_or("default")
            .to_string();
        let version = map_get_str(props, "version")
            .unwrap_or("1.0")
            .to_string();
        ...
        if let Some(Value::List(cl)) = map_get(props, "classes") {
            for item in cl {
                if let Value::Map(cm) = item {
                    ...
                    classes.insert(name.clone(), ClassDef { name, parent, description });
                }
            }
        }
            ...
            for item in rl {
                if let Value::Map(rm) = item {
                    let name = map_get_str(rm, "name")
                        .ok_or_else(|| KError::InvalidObject("ontology relationship missing name".into()))?
                        .to_string();
                    let domain = map_get_str(rm, "domain").map(String::from);
                    let range = map_get_str(rm, "range").map(String::from);
                    let cardinality = map_get_str(rm, "cardinality")
                        .and_then(Cardinality::from_str);
                    relationships.insert(name.clone(), RelDef { name, domain, range, cardinality });
                }
            }
        }
            ...
            for item in pl {
                if let Value::Map(pm) = item {
                    let name = map_get_str(pm, "name")
                        .ok_or_else(|| KError::InvalidObject("ontology property_def missing name".into()))?
                        .to_string();
                    let value_type = map_get_str(pm, "type")
                        .unwrap_or("Text")
                        .to_string();
                    let required = map_get_bool(pm, "required").unwrap_or(false);
                    property_defs.insert(name.clone(), PropertyDef { name, value_type, required });
                }
            }
        }
        ...
        if let Some(Value::List(ml)) = map_get(props, "mappings") {
            for item in ml {
                if let Value::Map(mm) = item {
                    let source = map_get_str(mm, "source")
                        .unwrap_or("unknown")
                        .to_string();
                    let physical_type = map_get_str(mm, "physical_type")
                        .unwrap_or("unknown")
                        .to_string();
                    let class = map_get_str(mm, "class")
                        .unwrap_or("unknown")
                        .to_string();
                    ...
                    mappings.push(MappingEntry { source, physical_type, class, property_map });
                }
            }
        }

        let def = OntologyDef { namespace, version, classes, relationships, property_defs, mappings };
        def.validate()?;
        Ok(def)
    }
    ...
    pub fn validate(&self) -> KResult<()> {
        if self.namespace.trim().is_empty() {
            return Err(KError::InvalidObject("ontology namespace must be non-empty".into()));
        }
        if self.classes.is_empty() {
            return Err(KError::InvalidObject("ontology must define at least one class".into()));
        }
        ...
        for (name, cd) in &self.classes {
            if let Some(ref parent) = cd.parent {
                if !self.classes.contains_key(parent) {
                    return Err(KError::InvalidObject(format!(
                        "ontology class '{}' has unknown parent '{}'", name, parent
                    )));
                }
            }
        }
        ...
        for (name, rd) in &self.relationships {
            if let Some(ref domain) = rd.domain {
                if !self.classes.contains_key(domain) {
                    return Err(KError::InvalidObject(format!(
                        "ontology relationship '{}' has unknown domain '{}'", name, domain
                    )));
                }
            }
            if let Some(ref range) = rd.range {
                if !self.classes.contains_key(range) {
                    return Err(KError::InvalidObject(format!(
                        "ontology relationship '{}' has unknown range '{}'", name, range
                    )));
                }
            }
        }
...
impl OntologyRegistry {
    ...
    pub fn new(def: OntologyDef) -> Result<Self, String> {
        ...
        for (i, me) in def.mappings.iter().enumerate() {
            by_physical.insert(
                (me.source.clone(), me.physical_type.clone()),
                i,
            );
        }
    ...
    pub fn map_external(&self, source: &str, physical_type: &str) -> Option<&MappingEntry> {
        let idx = self.by_physical.get(&(source.to_string(), physical_type.to_string()))?;
    ...
    pub fn register_from_ko(&mut self, ko: &KnowledgeObject) -> KResult<&OntologyDef> {
        ...
        *self = OntologyRegistry::new(new_def).map_err(|e| {
            KError::InvalidObject(format!("ontology reload failed: {}", e))
        })?;
        Ok(&self.def)
    }
}
...
pub fn discover_ontology(kos: &[KnowledgeObject]) -> OntologyDef {
    ...
    for ko in kos {
        ...
        let source = ko.metadata.tags.iter()
            .find(|t| t.starts_with("source:"))
            .map(|t| t.trim_start_matches("source:").to_string())
            .unwrap_or_else(|| "native".to_string());

        let entry = &mut type_props.entry(ko.metadata.type_name.clone()).or_insert_with(|| (source, BTreeMap::new())).1;
    ...
    for (type_name, (source, props)) in &type_props {
        classes.insert(type_name.clone(), ClassDef {
            name: type_name.clone(),
            parent: None,
            description: Some(format!("Auto-discovered from {} (source: {})", type_name, source)),
        });
        for (prop_name, vt) in props {
            property_defs.entry(prop_name.clone()).or_insert_with(|| PropertyDef {
    ...
    for (src_type, rel_name) in &rel_pairs {
        relationships.entry(rel_name.clone()).or_insert_with(|| RelDef {
            name: rel_name.clone(),
...
mod tests {
    ...
    fn sample_ontology() -> OntologyDef {
        ...
        classes.insert("Person".into(), ClassDef {
            name: "Person".into(), parent: None,
            description: Some("A human being".into()),
        });
        classes.insert("Employee".into(), ClassDef {
            name: "Employee".into(), parent: Some("Person".into()),
            description: Some("A person employed by a company".into()),
        });
        classes.insert("Manager".into(), ClassDef {
            name: "Manager".into(), parent: Some("Employee".into()),
            description: Some("An employee who manages others".into()),
        });
        classes.insert("Department".into(), ClassDef {
            name: "Department".into(), parent: None,
            description: Some("An organizational unit".into()),
        });

        let mut relationships = BTreeMap::new();
        relationships.insert("belongsTo".into(), RelDef {
            name: "belongsTo".into(),
        ...
        property_defs.insert("id".into(), PropertyDef {
            name: "id".into(), value_type: "Text".into(), required: true,
        });
        property_defs.insert("dept".into(), PropertyDef {
            name: "dept".into(), value_type: "Text".into(), required: false,
        });
    ...
    fn round_trip_through_ko() {
        ...
        let ko = KnowledgeObject::new(
            ...
            SecurityDescriptor { owner: "system".into(), acl: vec![], classification: None },
        );
        let mut ko = ko;
    ...
    fn conform_renames_properties() {
        ...
        ko.properties.insert("employee_id".into(), Value::Text("E123".into()));
        ko.properties.insert("department_id".into(), Value::Text("D10".into()));
        ko.properties.insert("name".into(), Value::Text("Alice".into()));

        conform(&mut ko, &r);

        // employee_id → id, department_id → dept, name unchanged
        assert_eq!(ko.properties.get("id").and_then(|v| match v { Value::Text(s) => Some(s.as_str()), _ => None }), Some("E123"));
        assert_eq!(ko.properties.get("dept").and_then(|v| match v { Value::Text(s) => Some(s.as_str()), _ => None }), Some("D10"));
        assert_eq!(ko.properties.get("name").and_then(|v| match v { Value::Text(s) => Some(s.as_str()), _ => None }), Some("Alice"));
    ...
    fn conform_untouched_for_unmapped_type() {
        ...
        ko.properties.insert("foo".into(), Value::Text("bar".into()));
    ...
    fn edge_empty_ontology_rejected_by_validate() {
        ...
        let def = OntologyDef {
            namespace: "empty".into(), version: "0".into(),
            classes: BTreeMap::new(),
            relationships: BTreeMap::new(),
            property_defs: BTreeMap::new(),
            mappings: vec![],
    ...
    fn edge_ontology_defaults_missing_fields() {
        ...
        let mut classes: BTreeMap<String, ClassDef> = BTreeMap::new();
        classes.insert("A".into(), ClassDef { name: "A".into(), parent: None, description: None });
        let mut props = PropertyMap::new();
        props.insert("namespace".into(), Value::Text("test".into()));
        props.insert("version".into(), Value::Text("1".into()));
        props.insert("classes".into(), Value::List(vec![Value::Map({
    ...
    fn edge_validate_rejects_empty_namespace() {
        let def = OntologyDef {
            namespace: "  ".into(), version: "1".into(),
            classes: {
                let mut m = BTreeMap::new();
                m.insert("A".into(), ClassDef { name: "A".into(), parent: None, description: None });
                m
            },
            relationships: BTreeMap::new(),
            property_defs: BTreeMap::new(),
    ...
    fn edge_validate_allows_wildcard_domain_and_range() {
        ...
        let mut classes = BTreeMap::new();
        classes.insert("Thing".into(), ClassDef { name: "Thing".into(), parent: None, description: None });
        let mut rels = BTreeMap::new();
        rels.insert("relatedTo".into(), RelDef { name: "relatedTo".into(), domain: None, range: None, cardinality: None });
        let def = OntologyDef {
            namespace: "test".into(), version: "1".into(),
            classes, relationships: rels,
            property_defs: BTreeMap::new(), mappings: vec![],
        };
    ...
    fn edge_discover_ontology_skips_builtin_types() {
        ...
        kos.push(KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: ONTOLOGY_TYPE.into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None }));
        // mnemosyne:* types should be skipped.
        kos.push(KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "mnemosyne:role".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None }));
        // Regular type should be included.
        let mut ko = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "Widget".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko.properties.insert("color".into(), Value::Text("red".into()));
        kos.push(ko);
    ...
    fn edge_discover_ontology_detects_source_from_tags() {
        let mut kos = vec![];
        let mut ko1 = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "t1".into(), tenant: None, schema_version: 1, tags: vec!["source:postgres".into()] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko1.properties.insert("a".into(), Value::Text("x".into()));
        kos.push(ko1);

        let mut ko2 = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "t2".into(), tenant: None, schema_version: 1, tags: vec!["source:mongodb".into()] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko2.properties.insert("b".into(), Value::Int(42));
        kos.push(ko2);

        // No source tag — should fall back to "native".
        let mut ko3 = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "t3".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ...
        let pg = def.mappings.iter().find(|m| m.physical_type == "t1").unwrap();
        assert_eq!(pg.source, "postgres");
        let mongo = def.mappings.iter().find(|m| m.physical_type == "t2").unwrap();
        assert_eq!(mongo.source, "mongodb");
        let native = def.mappings.iter().find(|m| m.physical_type == "t3").unwrap();
    ...
    fn edge_discover_ontology_infers_property_types() {
        let mut ko = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "Test".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko.properties.insert("bool_prop".into(), Value::Bool(true));
        ko.properties.insert("int_prop".into(), Value::Int(42));
        ko.properties.insert("float_prop".into(), Value::Float(3.14));
        ko.properties.insert("text_prop".into(), Value::Text("hello".into()));
        ko.properties.insert("null_prop".into(), Value::Null);
        ko.properties.insert("list_prop".into(), Value::List(vec![]));
        ko.properties.insert("map_prop".into(), Value::Map(BTreeMap::new()));
    ...
    fn edge_discover_ontology_detects_relationships() {
        let mut ko = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "Employee".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko.relationships.push(RelationshipRef {
            rel_type: "worksFor".into(), target: KOID::ZERO,
            direction: crate::knowledge::kom::Direction::Outbound,
        });
        ko.relationships.push(RelationshipRef {
            rel_type: "manages".into(), target: KOID::ZERO,
            direction: crate::knowledge::kom::Direction::Outbound,
        });
        ko.properties.insert("name".into(), Value::Text("Alice".into()));
    ...
    fn edge_discover_ontology_cumulative_across_sources() {
        ...
        let mut ko_pg = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "pg_emp".into(), tenant: None, schema_version: 1, tags: vec!["source:postgres".into()] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko_pg.properties.insert("emp_id".into(), Value::Text("E1".into()));

        let mut ko_neo = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "neo_emp".into(), tenant: None, schema_version: 1, tags: vec!["source:neo4j".into()] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko_neo.properties.insert("employeeId".into(), Value::Text("N1".into()));
        ko_neo.relationships.push(RelationshipRef {
            rel_type: "WORKS_FOR".into(), target: KOID::ZERO,
            direction: crate::knowledge::kom::Direction::Outbound,
        });
    ...
    fn edge_conform_preserves_unmapped_properties() {
        let r = sample_registry();
        let mut ko = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "employees".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko.properties.insert("employee_id".into(), Value::Text("E1".into()));
        ko.properties.insert("extra_field".into(), Value::Text("keep_me".into())); // no mapping
        ko.properties.insert("department_id".into(), Value::Text("D1".into()));
        ...
        assert_eq!(ko.properties.get("extra_field").and_then(text_val), Some("keep_me"));
    }
    ...
    fn edge_round_trip_optional_description_fields() {
        ...
        let mut classes = BTreeMap::new();
        classes.insert("A".into(), ClassDef { name: "A".into(), parent: None, description: Some("has desc".into()) });
        classes.insert("B".into(), ClassDef { name: "B".into(), parent: Some("A".into()), description: None });

        let def = OntologyDef {
            namespace: "t".into(), version: "1".into(),
            classes, relationships: BTreeMap::new(),
            property_defs: BTreeMap::new(), mappings: vec![],
        };
        let props = def.to_property_map();
        let def2 = OntologyDef::from_property_map(&props).unwrap();
        assert_eq!(def2.classes.get("A").unwrap().description.as_deref(), Some("has desc"));
    ...
    fn edge_conform_empty_property_map_no_rename() {
        let r = OntologyRegistry::new(OntologyDef {
            namespace: "t".into(), version: "1".into(),
            classes: { let mut m = BTreeMap::new(); m.insert("E".into(), ClassDef { name: "E".into(), parent: None, description: None }); m },
            relationships: BTreeMap::new(), property_defs: BTreeMap::new(),
            mappings: vec![MappingEntry { source: "pg".into(), physical_type: "emp".into(), class: "E".into(), property_map: BTreeMap::new() }],
        }).unwrap();

        let mut ko = KnowledgeObject::new(KOID::ZERO,
            Metadata { type_name: "emp".into(), tenant: None, schema_version: 1, tags: vec![] },
            SecurityDescriptor { owner: "s".into(), acl: vec![], classification: None });
        ko.properties.insert("foo".into(), Value::Text("bar".into()));
        let orig = ko.properties.clone();
    ...
    fn text_val(v: &Value) -> Option<&str> {
        match v { Value::Text(s) => Some(s.as_str()), _ => None }
    }
}
