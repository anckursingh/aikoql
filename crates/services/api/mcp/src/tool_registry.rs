...
use crate::session::*;
use crate::tools::*;
use crate::{error_codes, json, Kernel, J};
...
pub(crate) fn tools_list() -> J {
    ...
    json!({
        "tools": [
            ...
            {"name": "supersede", "description": "Replace a claim with a new generation. Preserves the old KO (Superseded + valid_to + SUPERSEDES edge) and sweeps its derived dependents for staleness (invalidation stamp). With superseded_by naming an existing successor, no new KO is created — the old claim is stamped Superseded with the SUPERSEDES edge to it (type_name/properties then unused).", "inputSchema": {"type": "object", "properties": {"subject": subj, "old": koid, "type_name": {"type": "string", "description": "Type of the new generation (required unless superseded_by is given)"}, "properties": {"type": "object"}, "evidence": {"type": "array", "items": {"type": "object"}, "description": "Canonical evidence entries: {source_artifact, method, location?, revision?, confidence?}"}, "reason": {"type": "string"}, "note": {"type": "string"}, "superseded_by": {"type": "string", "description": "KOID of an existing successor: stamp the old claim Superseded + SUPERSEDES edge to it instead of creating a new generation"}}, "required": ["old", "evidence"]}},
            ...
            {"name": "split_entity", "description": "Split a merged entity into two. Side A keeps its KOID, version lineage and everything not moved; side B is a new KO derived from it (DERIVED_FROM edge, inherited evidence and confidence, derivation op 'split'). Properties and relationships moved to B must exist on the subject. (QA2-KNOW-006)", "inputSchema": {"type": "object", "properties": {"subject": subj, "koid": koid, "expected_version": {"type": "integer"}, "properties": {"type": "object", "description": "Side B's full property set (keys must exist on the subject)"}, "relationships": {"type": "array", "items": {"type": "object", "properties": {"rel_type": {"type": "string"}, "target": koid}}, "description": "Caller-owned edges moved to side B"}, "reason": {"type": "string", "description": "Why these are distinct entities (recorded in the derivation)"}, "idempotency_key": {"type": "string"}}, "required": ["koid", "reason"]}},
            ...
            {"name": "resolve_conflict", "description": "Apply a resolution decision to a persisted Conflict KO: resolved_a_preferred | resolved_b_preferred | resolved_both_valid | resolved_replaced. Rationale is mandatory — the kernel never silently picks a side. For resolved_both_valid, split_at partitions validity: claim A valid until, claim B valid from.", "inputSchema": {"type": "object", "properties": {"subject": subj, "koid": koid, "decision": {"type": "string", "enum": ["resolved_a_preferred", "resolved_b_preferred", "resolved_both_valid", "resolved_replaced"]}, "rationale": {"type": "string"}, "replacement": {"type": "string", "description": "Replacement claim KOID (required for resolved_replaced)"}, "split_at": {"type": "integer", "description": "Epoch millis validity partition for resolved_both_valid: claim A closes at this instant, claim B opens there (error with any other decision)"}}, "required": ["koid", "decision", "rationale"]}},
            ...
            {"name": "evidence_pack", "description": "Generate an exportable GDPR/HIPAA compliance evidence pack: audit chain, object inventory, PII-filtering config, retention records, and the encryption compliance report (MRFC-0020 Phase 4).", "inputSchema": {"type": "object", "properties": {"framework": {"type": "string", "description": "Compliance framework label: gdpr or hipaa"}}, "required": ["framework"]}},
            {"name": "reason", "description": "Submit a Class-B reasoning job (MRFC-0011 §6.10): find objects matching properties and produce provenance-tagged claims. Returns a job handle — poll with job_status, then approve_job to commit the claims (origin=reason, epistemic=inferred). Over the admission limit the submit is JOB_REJECTED. Idempotent: the same input returns the same job (possibly an already-completed one) — change the input to re-reason.", "inputSchema": {"type": "object", "properties": {"type_name": {"type": "string"}, "properties": {"type": "object"}}, "required": ["type_name"]}},
            ...
            {"name": "execute_program", "description": "Execute a deployed program KO by KOID with optional parameters. Pass execution_id for exactly-once execution: the result is snapshotted and replays with the same id return it without re-running.", "inputSchema": {"type": "object", "properties": {"koid": {"type": "string"}, "params": {"type": "object"}, "execution_id": {"type": "string"}}, "required": ["koid"]}},
...
pub(crate) fn call_tool(
    ...
    let res = match name {
        ...
        "summarize_conversation" => tool_summarize_conversation(k, args),
        ...
        "split_entity" => tool_split_entity(k, args),
        ...
        "restore" => tool_restore(args, admin),
        ...
        "evidence_pack" => tool_evidence_pack(k, args),
        "reason" => tool_reason(k, args),
        "infer" => tool_infer(k, args),
        "predict" => tool_predict(k, args),
        "job_status" => tool_job_status(k, args),
        "approve_job" => tool_approve_job(k, args),
        ...
        "register_schema" => tool_register_schema(k, args),
